...
export function evaluateExtractionSafety(extraction: MessageExtraction): SafetyDecision {
  if (ALWAYS_HUMAN_INTENTS.has(extraction.intent)) {
    return { allowed: false, requiresHuman: true, priority: true, reason: extraction.intent === 'complaint' ? 'complaint_requires_human' : 'order_status_requires_identity_check', askForMarket: false }
  }
  if (extraction.requiresHuman) {
    return { allowed: false, requiresHuman: true, priority: true, reason: 'model_requested_human_review', askForMarket: false }
  }
  if (extraction.confidence < 0.75) {
    return { allowed: false, requiresHuman: true, priority: true, reason: 'low_confidence', askForMarket: false }
  }
  if (['product_availability', 'product_price', 'product_sizing'].includes(extraction.intent) && extraction.candidateProductNames.length === 0) {
    return { allowed: false, requiresHuman: true, priority: false, reason: 'product_not_identified', askForMarket: false }
  }
  if (MARKET_REQUIRED_INTENTS.has(extraction.intent) && !extraction.market) {
    return { allowed: false, requiresHuman: false, priority: false, reason: 'market_required', askForMarket: true }
  }
  if (extraction.candidateProductNames.length > 1) {
    return { allowed: false, requiresHuman: true, priority: false, reason: 'multiple_products_identified', askForMarket: false }
  }
  return { allowed: true, requiresHuman: false, priority: false, reason: 'eligible_for_retrieval', askForMarket: false }
}
