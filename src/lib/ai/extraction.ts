...
export const EXTRACTION_RESPONSE_SCHEMA = {
  ...
  schema: {
    ...
    required: ['intent', 'language', 'candidateProductNames', 'size', 'colour', 'market', 'confidence', 'requiresHuman'],
    properties: {
...
export function buildExtractionRequest(body: string, model = 'gpt-5.4-nano'): AiRequest {
  return {
    model,
...
export async function extractInstagramMessage(provider: AiProvider, body: string, model = 'gpt-5.4-nano'): Promise<{ extraction: MessageExtraction; usage?: Awaited<ReturnType<AiProvider['complete']>>['usage']; requestId?: string }> {
  if (!body.trim()) throw new Error('Cannot extract an empty Instagram message')
  const response = await provider.complete<unknown>(buildExtractionRequest(body, model))
  return { extraction: parseMessageExtraction(response.output), usage: response.usage, requestId: response.requestId }
}
